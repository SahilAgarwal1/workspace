...
extern "C" void DMA_INT1_IRQHandler(void)
{
    ...
    if(g_audio_capture_first_time) g_audio_capture_first_time = false;

    // otherwise, increment capture buffer head
    else incCircBufferIndex((int16_t *)&g_audio_capture_circ_write_index, audioCapBuffSliceCount);

    // increment the timestamp

    g_latest_audio_timestamp += 30;
                             ...
                             int *audio_samples_size, int16_t **audio_samples)
{
    ...
    * 160 ), first 160 samples (320 bytes) will be from history */
    ...
    memset(&g_audio_output_buffer[480], 0, 2 * (kMaxAudioSampleSize - 320 - 160)); // 2 multiply for bytes left
...
static void ADC_init(void)
{
    ...
    P4 -> SEL0 &= ~BIT1; // GPIO
    P4 -> SEL1 &= ~BIT1; // GPIO
    P4 -> DIR |= BIT1; // 4.1 output

    P4 -> SEL0 |= BIT3; // ANALOG INPUT
    P4 -> SEL1 |= BIT3; // ANALOG INPUT
    P4-> DIR &= ~BIT3; // 4.3 input

    /* Turning on the microphone */
	// To implement

    P4 -> OUT |= BIT1; // turn on the microphone
