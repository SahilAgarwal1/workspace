...
interrupt void MRINTB_isr(void){
    ...
    GPIO_setPortPins(GPIO_PORT_A, 0x0F);
            ...
            if(record == 0){
                 uint32_t temp;
                 McBSP_receive32BitDataNonBlocking(MCBSPB_BASE, &temp);
                 McBSP_transmit32BitDataBlocking(MCBSPB_BASE, SRAM_16bit_read(circle_buff_tail & 0xFFFF));
                 circle_buff_tail++;
            ...
            if(ch_sel == 0){
                ...
                if(record == 1){
                    if(!switchRead) {
                        ...
                        SRAM_16bit_write(circle_buff_head & 0xFFFF, (uint16_t)(sample_L >>16));
                    }
                    else{
                        GPIO_clearPortPins(GPIO_PORT_A, 0x09);
                        uint16_t temp = SRAM_16bit_read(circle_buff_head & 0xFFFF);
                        SRAM_16bit_write(circle_buff_head & 0xFFFF, ((uint16_t)(sample_L >>16)) + temp);
                    }
...
void playFromBuffer(){
    uint32_t readData;
    GPIO_WritePin(66, 0); // bring cs low
    Spi_Transmit((0x03 << 8) | ((circle_buff_tail  & 0x00FF0000) >> 16));
    Spi_Transmit(circle_buff_tail & 0x0000FFFF);

    while(circle_buff_tail != circle_buff_head){
        readData = Spi_Transmit(0x0000);
        readData = (readData << 16) | (Spi_Transmit(0x0000));
        McBSP_transmit32BitDataBlocking(MCBSPB_BASE, (readData & 0x00FFFF00) << 8);
        circle_buff_tail++;
        circle_buff_tail = circle_buff_tail % circle_buff_max;
    }
    GPIO_WritePin(66, 1); // set chip select high
}
