...
void initSequentialMode(){
    GPIO_WritePin(66, 0);
    Spi_Transmit((0x01 << 8) | 0b01000000);
    GPIO_WritePin(66, 1);
}
...
void SRAM_16bit_write(uint32_t address, uint16_t data){ // write using sequential

    GPIO_WritePin(66, 0); // set chip select low
    Spi_Transmit((0x02 << 8) | ((address  & 0x00FF0000) >> 16));
    Spi_Transmit(address & 0x0000FFFF);
    Spi_Transmit(data);
...
uint16_t SRAM_16bit_read(uint32_t address){
    GPIO_WritePin(66, 0); // bring cs low
    Spi_Transmit((0x03 << 8) | ((address  & 0x00FF0000) >> 16));
    Spi_Transmit(address & 0x0000FFFF);
    uint16_t readData = Spi_Transmit(0x0000);
    readData = (readData << 8) | (Spi_Transmit(0x0000) >> 8);
    GPIO_WritePin(66, 1); // set chip select high
    return readData;
}
